...
const Sidebar: React.FC = () => {
  const navItems = [
    { name: 'Dashboard', path: '/', icon: 'grid-view' },
    { name: 'Care Directory', path: '/directory', icon: 'search' },
    { name: 'AI Pharmacy', path: '/pharmacy', icon: 'medkit' },
    { name: 'Health Vault', path: '/vault', icon: 'description' },
    { name: 'Insurance', path: '/insurance', icon: 'shield' },
    { name: 'My Health', path: '/profile', icon: 'person' },
  ];
  ...
  return (
    <aside className="w-64 bg-white border-r border-gray-100 flex flex-col h-full hidden md:flex">
      <div className="p-6 flex items-center gap-2">
        ...
        <span className="text-2xl font-bold text-gray-800">Care India</span>
      </div>
            ...
            className={({ isActive }) =>
              `flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-200 ${
                isActive
                  ...
                  : 'text-gray-500 hover:bg-gray-50 hover:text-[#0066FF]'
              }`
            }
          >
             <Icon name={item.icon} />
